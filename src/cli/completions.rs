...
//! `fpath` at all. So `minato completions <shell>` still prints the script to
//! standard output and the destination to standard error, and
//! `minato completions <shell> --install` carries it out.
//!
...
pub const DOCS: &str = "https://m.canouil.dev/minato/shells.html";

/// What opens the block this installer manages in a shell's configuration.
const BLOCK_START: &str = "# >>> minato completions >>>";

/// What closes it.
const BLOCK_END: &str = "# <<< minato completions <<<";

/// Anything that stops a completion script from being printed or installed.
#[derive(Debug, thiserror::Error)]
pub enum CompletionsError {
    /// There is no home directory to resolve a destination against.
    ...
    UnknownShell {
        /// What `$SHELL` held, when it held anything.
        found: Option<String>,
    },

    /// The shell reads its completions from somewhere other than a file.
    #[error(
        "{shell} reads completions from its profile rather than from a file, so there is nothing to install.\n{hint}"
    )]
    NotInstallable {
...
impl Environment {
    ...
    /// # Errors
    ///
    /// Returns [`CompletionsError::NoHome`] when `$HOME` is unset, since every
    /// destination is named relative to it.
    pub fn from_env() -> Result<Self, CompletionsError> {
        let home = std::env::var_os("HOME")
            .map(PathBuf::from)
            .filter(|home| !home.as_os_str().is_empty())
            .ok_or(CompletionsError::NoHome)?;
...
pub fn known_locations(environment: &Environment, shell: Shell) -> Vec<Location> {
    match shell {
        ...
        Shell::Fish => vec![Location {
            shell,
            path: environment.config.join("fish/completions/minato.fish"),
        ...
        Shell::Elvish => vec![Location {
            shell,
            path: environment.config.join("elvish/lib/minato.elv"),
            convention: "elvish/lib",
            rc: Some(RcEdit {
                file: environment.config.join("elvish/rc.elv"),
                body: "use minato".to_owned(),
            }),
            sweep_only: false,
        }],
...
fn bash_locations(environment: &Environment) -> Vec<Location> {
    let sourced = environment.data.join("minato-completions/minato.bash");
    ...
    vec![
        Location {
            ...
            // own is enough.
            path: environment.data.join("bash-completion/completions/minato"),
...
fn zsh_locations(environment: &Environment) -> Vec<Location> {
    ...
    // `_minato` would never be picked up. `-i` keeps the check and only skips
    ...
    let mut locations = vec![Location {
        ...
        path: environment.oh_my_zsh.join("completions/_minato"),
    ...
    locations.push(Location {
        shell: Shell::Zsh,
        path: site_functions.join("_minato"),
...
fn is_writable(directory: &Path) -> bool {
    let probe = directory.join(".minato-write-test");
...
/// # Errors
///
/// Returns [`CompletionsError::NotInstallable`] for a shell that reads its
/// completions from somewhere other than a file.
pub fn plan(environment: &Environment, shell: Shell) -> Result<Plan, CompletionsError> {
    let locations = known_locations(environment, shell);
    let not_installable = || CompletionsError::NotInstallable {
        shell,
        hint: hint(shell),
    };
    ...
    let chosen = match existing {
        ...
        None => default_location(environment, shell).ok_or_else(not_installable)?,
    };
    ...
    Ok(Plan {
        stale_rc: stale_rc(&locations, chosen.rc.as_ref()),
...
/// not want.
fn stale_rc(locations: &[Location], keeping: Option<&RcEdit>) -> Option<PathBuf> {
    locations
        .iter()
        .filter_map(|location| location.rc.as_ref())
        .map(|rc| rc.file.clone())
        .find(|file| keeping.is_none_or(|keeping| &keeping.file != file) && block_present(file))
}
...
/// # Errors
///
/// Returns [`CompletionsError::Io`] when a file cannot be written or removed.
pub fn install(plan: &Plan, script: &str) -> Result<String, CompletionsError> {
...
/// # Errors
///
/// Returns [`CompletionsError::Io`] when a file cannot be removed or rewritten.
pub fn uninstall(
    ...
    {
        if !blocks.contains(&file) && block_present(&file) {
            blocks.push(file);
        }
    }
...
pub fn shell_from_env() -> Result<Shell, CompletionsError> {
    ...
    match name {
        ...
        _ => Err(CompletionsError::UnknownShell { found }),
    }
}
...
/// # Panics
///
/// Panics if a generator emits something other than UTF-8, which none of them
/// does: every one writes its own source, and the only thing interpolated into
/// it comes from a command definition built from `&str`.
#[must_use]
...
#[must_use]
pub fn hint(shell: Shell) -> String {
    let steps = match shell {
        Shell::Bash => [
            "Install it with `minato completions bash --install`, which writes it to",
            "~/.local/share/bash-completion/completions when that directory exists,",
            "and to ~/.local/share/minato-completions with a line in ~/.bashrc otherwise.",
        ]
        .join("\n"),
        // Named in the order an install prefers them, so the first line that
        // applies is the one to read.
        Shell::Zsh => [
            "Install it with `minato completions zsh --install`, which writes it as",
            "_minato in a directory on $fpath:",
            "  ~/.oh-my-zsh/custom/completions under oh-my-zsh, which needs nothing further,",
            "  $HOMEBREW_PREFIX/share/zsh/site-functions under Homebrew, likewise,",
            "  ~/.zfunc otherwise, with this added to ~/.zshrc:",
            "    fpath=(~/.zfunc $fpath)",
            "    autoload -Uz compinit && compinit -i",
        ]
        .join("\n"),
        Shell::Fish => [
            "Install it with `minato completions fish --install`, or write it to",
            "~/.config/fish/completions/minato.fish yourself.",
        ]
        .join("\n"),
        Shell::Elvish => [
            "Install it with `minato completions elvish --install`, which writes it to",
            "~/.config/elvish/lib/minato.elv and loads it from rc.elv:",
            "  use minato",
        ]
        .join("\n"),
        Shell::PowerShell => [
...
fn block_present(file: &Path) -> bool {
    std::fs::read_to_string(file)
        .is_ok_and(|contents| contents.lines().any(|line| line.trim_end() == BLOCK_START))
}

/// Writes the managed block into `file`, replacing any block already there.
fn write_block(file: &Path, body: &str) -> Result<(), CompletionsError> {
    let existing = std::fs::read_to_string(file).unwrap_or_default();
    let mut kept = without_block(&existing);
    ...
    write_file(file, &format!("{kept}{BLOCK_START}\n{body}\n{BLOCK_END}\n"))
}

/// Removes the managed block from `file`, leaving everything else alone.
fn remove_block(file: &Path) -> Result<(), CompletionsError> {
    let Ok(existing) = std::fs::read_to_string(file) else {
        return Ok(());
    };

    write_file(file, &without_block(&existing))
}
...
fn create_directory(directory: &Path) -> Result<(), CompletionsError> {
    std::fs::create_dir_all(directory).map_err(|source| CompletionsError::Io {
        path: directory.to_path_buf(),
        source,
    })
}

fn write_file(path: &Path, contents: &str) -> Result<(), CompletionsError> {
    std::fs::write(path, contents).map_err(|source| CompletionsError::Io {
        path: path.to_path_buf(),
        source,
    })
}

fn remove_file(path: &Path) -> Result<(), CompletionsError> {
    std::fs::remove_file(path).map_err(|source| CompletionsError::Io {
        path: path.to_path_buf(),
        source,
    })
}
...
mod tests {
    ...
    fn a_first_zsh_install_goes_to_zfunc() {
        ...
        assert_eq!(plan.location.path, home.path().join(".zfunc/_minato"));
    ...
    fn oh_my_zsh_wins_when_it_is_installed_and_needs_no_configuration() {
        ...
        assert_eq!(
            plan.location.path,
            environment.oh_my_zsh.join("completions/_minato")
        );
    ...
    fn homebrew_wins_over_zfunc_and_needs_no_configuration() {
        ...
        assert_eq!(
            plan.location.path,
            prefix.join("share/zsh/site-functions/_minato")
        );
    ...
    fn an_existing_install_is_kept_where_it_is() {
        let home = tempfile::tempdir().expect("a temporary home");
        let environment = environment(home.path());
        plant(&home.path().join(".zfunc/_minato"));
        std::fs::create_dir_all(&environment.oh_my_zsh).expect("an oh-my-zsh layout");
    ...
    fn the_copies_that_are_not_kept_are_swept() {
        let home = tempfile::tempdir().expect("a temporary home");
        let environment = environment(home.path());
        let omz = environment.oh_my_zsh.join("completions/_minato");
        let xdg = home.path().join(".local/share/zsh/site-functions/_minato");
        plant(&omz);
        plant(&xdg);
    ...
    fn installing_writes_the_script_and_the_block_and_sweeps_the_rest() {
        ...
        let plan = plan(&environment, Shell::Zsh).expect("a plan");
        install(&plan, "#compdef minato\n").expect("the install to succeed");

        let installed = home.path().join(".zfunc/_minato");
        assert_eq!(
    ...
    fn re_running_replaces_the_block_rather_than_appending_one() {
        ...
        let zshrc = std::fs::read_to_string(home.path().join(".zshrc")).expect("the rc file");
        assert_eq!(zshrc.matches(BLOCK_START).count(), 1, "{zshrc}");
        assert_eq!(zshrc.matches(BLOCK_END).count(), 1, "{zshrc}");
        assert!(
    ...
    fn a_dry_run_writes_nothing() {
        ...
        assert!(!home.path().join(".zfunc/_minato").exists());
    ...
    fn uninstalling_removes_every_copy_and_the_block() {
        ...
        let plan = plan(&environment, Shell::Zsh).expect("a plan");
        install(&plan, "#compdef minato\n").expect("the install to succeed");
        plant(&home.path().join(".local/share/zsh/site-functions/_minato"));
        ...
        assert!(
            !home
                .path()
                .join(".local/share/zsh/site-functions/_minato")
                .exists()
        );
        let zshrc = std::fs::read_to_string(home.path().join(".zshrc")).expect("the rc file");
        assert!(!zshrc.contains(BLOCK_START), "{zshrc}");
    }
    ...
    fn a_stale_copy_that_cannot_be_removed_does_not_fail_the_install() {
        ...
        plant(&directory.join("_minato"));
        std::fs::set_permissions(&directory, std::fs::Permissions::from_mode(0o500))
            .expect("making the directory read-only");

        let plan = plan(&environment, Shell::Zsh).expect("a plan");
        let report = install(&plan, "#compdef minato\n").expect("the install to succeed");
        ...
        assert!(home.path().join(".zfunc/_minato").is_file(), "{report}");
    }
    ...
    fn bash_takes_the_bash_completion_directory_only_when_it_exists() {
        ...
        assert_eq!(
            ...
            home.path()
                .join(".local/share/minato-completions/minato.bash")
        );
        ...
        assert_eq!(
            lazy.location.path,
            home.path()
                .join(".local/share/bash-completion/completions/minato")
        );
        assert!(
    ...
    fn every_installable_shell_names_its_file_per_convention() {
        ...
        assert!(names.contains(&"minato".to_owned()), "{names:?}");
        // The leading underscore, which is easy to leave off and leaves a file
        // zsh never reads.
        assert!(names.contains(&"_minato".to_owned()), "{names:?}");
        assert!(names.contains(&"minato.fish".to_owned()), "{names:?}");
        assert!(names.contains(&"minato.elv".to_owned()), "{names:?}");
    }
