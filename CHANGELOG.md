...
- fix: in the TUI, clear a status message on the next key so the key hints return, refresh the table after a fetch or update, and open the screen before the first scan so it no longer appears to hang on start (#PLACEHOLDER).
